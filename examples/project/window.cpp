...
void Window::onPaint() {
  ...
  for (int x = 0; x < cubeSideCnt; x++) {
    for (int y = 0; y < cubeSideCnt; y++) {
      glm::mat4 model{1.0f};
      model =
          glm::translate(model, glm::vec3(-(cubeSideCnt / 2.0 - 1.0f) + x, 0.5f,
  ...
  const int cubeSideCnto{15};
  for (int x = 0; x < cubeSideCnto; x++) {
    for (int y = 0; y < cubeSideCnto; y++) {
      glm::mat4 model{1.0f};
      model = glm::translate(model,
                             glm::vec3(-(cubeSideCnto / 2.0 - 3.0f) + x, 1.2f,
                                       -(cubeSideCnto / 2.0 - 5.0f) + y));
      ...
      abcg::glUniformMatrix4fv(m_modelMatrixLocation, 1, GL_FALSE,
                               &model[0][0]);
      abcg::glUniform4f(m_colorLocation, x / (cubeSideCnto * 1.0),
                        (x + y) / (cubeSideCnto * 2.0f),
                        y / (cubeSideCnto * 1.0), 1.0f);
      abcg::glDrawElements(GL_TRIANGLES, m_indices.size(), GL_UNSIGNED_INT,
                           nullptr);
    }
  }
